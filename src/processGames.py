...
def processGames(gameDetailIds, cursor):
    for gameDetailId in gameDetailIds:
        if gameDetailId == None:
            continue;
        ...
        if (response["status"] == 200):
            processGameData(response["results"], gameDetailId, cursor);
        else:
            print("Bad response: ", response["status"]);

def processGameData(gameData, gameDetailId, cursor):
